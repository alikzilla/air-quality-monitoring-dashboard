...
import { Activity, BellRing, Cog, FileClock, LayoutDashboard } from 'lucide-react';
    ...
    <aside className="sidebar">
      ...
      <nav className="sidebar__nav" aria-label="Main navigation">
        {navItems.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link key={href} href={href} className={cn('sidebar__link', active && 'sidebar__link--active')}>
              <Icon size={16} />
              <span>{label}</span>
            </Link>
          );
